...
public class StarSort implements HotelSort{
	...
	public ArrayList<HotelVO> getSortedList(HotelSearchCondition condition, ArrayList<HotelVO> hotels) {
		if (hotels == null || condition != HotelSearchCondition.STAR_UP
				|| condition != HotelSearchCondition.STAT_DOWN) {
	 ...
	 * @author all
	 */
	public ArrayList<HotelVO> getSortedList(int starStar, int endStar, HotelSearchCondition condition,
			ArrayList<HotelVO> hotels) {
		if (hotels == null || starStar > endStar || condition != HotelSearchCondition.STAR_UP
				|| condition != HotelSearchCondition.STAT_DOWN) {
			System.out.println("logic.hotel.starSort.getSortedList参数异常");
			return null;
		}
